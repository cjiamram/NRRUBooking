var regDec = /^\d+(\.\d{1,2})?$/;
var regEmail=/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g;
var regTel=/^[+]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*$/g;
var regDate=/(0[1-9]|1[012])[- \/.](0[1-9]|[12][0-9]|3[01])[- \/.](19|20)\d\d/;
function validInput(){
		var flag=true;
		flag=regDate.test($("#obj_bookingDate").val());
		if (flag==false){
				$("#obj_bookingDate").focus();
				return flag;
		}
		return flag;
}
function displayData(){
		var url="tzoombooking/displayData.php?tableName=t_zoombooking&dbName=dbzoombooking&keyWord="+$("#txtSearch").val();
		$("#tblDisplay").load(url);
}
function createData(){
		var url='tzoombooking/create.php';
		jsonObj={
			zoomCode:$("#obj_zoomCode").val(),
			bookingDate:$("#obj_bookingDate").val(),
			startTime:$("#obj_startTime").val(),
			finishTime:$("#obj_finishTime").val(),
			bookingName:$("#obj_bookingName").val(),
			staffId:$("#obj_staffId").val(),
			status:$("#obj_status").val(),
			activity:$("#obj_activity").val(),
			telNo:$("#obj_telNo").val(),
			lineNo:$("#obj_lineNo").val(),
			aproveStatus:$("#obj_aproveStatus").val(),
			outsiderId:$("#obj_outsiderId").val()
		}
		var jsonData=JSON.stringify (jsonObj);
		var flag=executeData(url,jsonObj,false);
		return flag;
}
function updateData(){
		var url='tzoombooking/update.php';
		jsonObj={
			zoomCode:$("#obj_zoomCode").val(),
			bookingDate:$("#obj_bookingDate").val(),
			startTime:$("#obj_startTime").val(),
			finishTime:$("#obj_finishTime").val(),
			bookingName:$("#obj_bookingName").val(),
			staffId:$("#obj_staffId").val(),
			status:$("#obj_status").val(),
			activity:$("#obj_activity").val(),
			telNo:$("#obj_telNo").val(),
			lineNo:$("#obj_lineNo").val(),
			aproveStatus:$("#obj_aproveStatus").val(),
			outsiderId:$("#obj_outsiderId").val(),
			id:$("#obj_id").val()
		}
		var jsonData=JSON.stringify (jsonObj);
		var flag=executeData(url,jsonObj,false);
		return flag;
}
function readOne(id){
		var url='tzoombooking/readOne.php?id='+id;
		data=queryData(url);
		if(data!=""){
			$("#obj_zoomCode").val(data.zoomCode);
			$("#obj_bookingDate").val(data.bookingDate);
			$("#obj_startTime").val(data.startTime);
			$("#obj_finishTime").val(data.finishTime);
			$("#obj_bookingName").val(data.bookingName);
			$("#obj_staffId").val(data.staffId);
			$("#obj_status").val(data.status);
			$("#obj_activity").val(data.activity);
			$("#obj_telNo").val(data.telNo);
			$("#obj_lineNo").val(data.lineNo);
			$("#obj_aproveStatus").val(data.aproveStatus);
			$("#obj_outsiderId").val(data.outsiderId);
			$("#obj_id").val(data.id);
		}
}
function saveData(){
		var flag;
		flag=validInput();
		if(flag==true){
					if($("#obj_id").val()!=""){
			flag=updateData();
			}else{
			flag=createData();
		}
		if(flag==true){
			swal.fire({
			title: "การบันทึกข้อมูลเสร็จสมบูรณ์แล้ว",
			type: "success",
			buttons: [false, "ปิด"],
			dangerMode: true,
		});
		displayData();
		}
		else{
			swal.fire({
			title: "การบันทึกข้อมูลผิดพลาด",
			type: "error",
			buttons: [false, "ปิด"],
			dangerMode: true,
		});
		}
		}else{
			swal.fire({
			title: "รูปแบบการกรอกข้อมูลไม่ถูกต้อง",
			type: "error",
			buttons: [false, "ปิด"],
			dangerMode: true,
			});
			}
}
function confirmDelete(id){
		swal.fire({
			title: "คุณต้องการที่จะลบข้อมูลนี้หรือไม่?",
			text: "***กรุณาตรวจสอบข้อมูลให้ครบถ้วนก่อนกดปุ่มตกลง",
			type: "warning",
			confirmButtonText: "ตกลง",
			cancelButtonText: "ยกเลิก",
			showCancelButton: true,
			showConfirmButton: true
		}).then((willDelete) => {
		if (willDelete.value) {
			url="tzoombooking/delete.php?id="+id;
			executeGet(url,false,"");
			displayData();
		swal.fire({
			title: "ลบข้อมูลเรียบร้อยแล้ว",
			type: "success",
			buttons: "ตกลง",
		});
		} else {
			swal.fire({
			title: "ยกเลิกการทำรายการ",
			type: "error",
			buttons: [false, "ปิด"],
			dangerMode: true,
		})
		}
		});
}
function clearData(){
			$("#obj_zoomCode").val("");
			$("#obj_bookingDate").val("");
			$("#obj_startTime").val("");
			$("#obj_finishTime").val("");
			$("#obj_bookingName").val("");
			$("#obj_staffId").val("");
			$("#obj_status").val("");
			$("#obj_activity").val("");
			$("#obj_telNo").val("");
			$("#obj_lineNo").val("");
			$("#obj_aproveStatus").val("");
			$("#obj_outsiderId").val("");
}
function genCode(){
		//var url="genCode.php";
		//var data=queryData(url);
		//return data.code;
}
